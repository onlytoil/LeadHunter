...
export function DashboardHeader({
  ...
  const statusClass = telegramStatus?.lastError
    ? "bg-red-400/15 text-red-300"
    : isConnected
      ? "bg-emerald-400/15 text-emerald-300"
      : "bg-amber-400/15 text-amber-300";

  const statusText = telegramStatus?.lastError
    ? "Telegram: ошибка"
    : isConnected
      ? "Telegram: подключён"
  ...
  return (
    <header className="mb-8">
      ...
      <p className="mt-3 max-w-2xl text-slate-400">
        Находи потенциальных клиентов в Telegram-чатах и управляй результатами
        в одном месте.
      </p>
